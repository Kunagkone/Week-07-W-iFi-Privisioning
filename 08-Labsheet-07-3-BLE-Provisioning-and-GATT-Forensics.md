...
1. เหตุใด BLE Provisioning จึงไม่ส่งผลให้สัญญาณ Wi-Fi บนสมาร์ตโฟนของผู้ใช้หลุดระหว่างทำรายการ?
   ```
   เนื่องจากกระบวนการรับส่งข้อมูลคอนฟิก (SSID และ Password) ทำผ่านอินเทอร์เฟซ Bluetooth Low Energy (BLE) ซึ่งเป็นโปรโตคอลและตัวรับส่งสัญญาณคนละส่วนกับเครือข่าย Wi-Fi บนสมาร์ตโฟน ทำให้สมาร์ตโฟนสามารถคงการเชื่อมต่อกับ Wi-Fi บ้านเดิม
   ```
2. Descriptor `0x2901` มีความสำคัญอย่างไรต่อการที่แอปพลิเคชันมือถือจะทราบว่า Characteristic แต่ละตัวใช้ทำหน้าที่อะไร?
   ```
   Descriptor 0x2901 (Characteristic User Description) ทำหน้าที่เก็บข้อความสายอักขระ (User String) ที่ระบุชื่อ Endpoint แบบอ่านเข้าใจได้ เช่น "prov-session", "prov-config" เมื่อแอปพลิเคชันมือถือเชื่อมต่อเข้ากับ GATT Server จะสามารถอ่านค่าใน Descriptor นี้เพื่อแมป (Map) ได้ทันทีว่า Characteristic แต่ละตำแหน่งใช้สำหรับทำหน้าที่อะไรในการสื่อสาร Protocomm โดยไม่จำเป็นต้อง ฮาร์ดโค้ด (Hardcode) UUID 128-bit ที่ยาวและจำยากไว้ในโค้ดของแอปมือถือ
   ```
3. การที่ ESP-IDF มีฟังก์ชัน `esp_bt_mem_release()` มีประโยชน์อย่างไรต่อการทำงานของแอปพลิเคชัน IoT หลังเชื่อมต่อ Wi-Fi สำเร็จ?
   ```
   ฟังก์ชัน esp_bt_mem_release() จะสั่งคืนพื้นที่หน่วยความจำ DRAM ที่เคยถูกจองไว้ใช้สำหรับ Bluetooth Controller และ BT Stack (BTDM / NimBLE) กลับคืนสู่ระบบ System Heap ส่งผลให้ไมโครคอนโทรลเลอร์ ESP32 ได้รับ RAM ว่างกลับคืนมามหาศาล (หลายสิบถึงร้อย Kilobytes) ซึ่งมีความสำคัญอย่างยิ่งต่อการนำไปใช้รันแอปพลิเคชันหลักของ IoT เช่น การเชื่อมต่อเข้ารหัส TLS/SSL, การรันระบบ MQTT, Web Server หรือกระบวนการประมวลผลข้อมูลที่ต้องใช้ RAM สูงต่อไป
   ```
